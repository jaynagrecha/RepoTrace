...
Current package: v0.27 v22.1 Production Hardening Notes
...
## v23 Full File Inventory + VT Hash Verdicts

RepoTrace now lists the selected repository file inventory rather than only high-risk files. For each selected file it attempts to calculate:

- MD5
- SHA1
- SHA256
- VirusTotal hash verdict when `VT_API_KEY` is configured
...
VT_API_KEY=
VT_LOOKUP_LIMIT=300
VT_LOOKUP_CONCURRENCY=3
MAX_FILE_HASH_BYTES=5000000
```

Notes:

- RepoTrace does not upload files to VirusTotal. It only performs hash lookups.
- Files larger than `MAX_FILE_HASH_BYTES` are listed but skipped for hashing to protect bandwidth and runtime.
- Very large repos should be scanned carefully because full file hashing can consume GitHub and VirusTotal API quota.

## v24 Additions

- Optional enterprise/org user registration and login.
- Logged-in org users can provide their own GitHub token so scans use their token instead of the public RepoTrace token.
- Basic report button to generate a structured suspicious repository/user report template.
- Reports are saved locally under `data/reports/` and can optionally be emailed using SMTP with `REPORT_TO_EMAIL`.

> Note: v24 keeps file-based user storage for MVP simplicity. For large public/enterprise deployment, move users/tokens to a proper database and secret manager.


## Enterprise / Org Login Token Routing

RepoTrace does **not** require enterprise users to paste GitHub secrets into the browser.

Instead, users can register/login with an organization email address, and RepoTrace can route scans to a server-side GitHub token configured by domain.

Supported environment formats:

```env
ORG_GITHUB_TOKEN_WU_COM=github_pat_xxx
```

or:

```env
ORG_GITHUB_TOKENS_JSON={"wu.com":"github_pat_xxx","example.com":"github_pat_yyy"}
```

If no org-specific token is configured, RepoTrace falls back to the public/default `GITHUB_TOKEN`.

## Reporting Suspicious Repos / Users

RepoTrace can generate a structured report template for suspicious repositories or users and optionally forward the report to an internal mailbox via SMTP.

GitHub abuse reports still require submission through GitHub's official reporting UI; RepoTrace prepares the evidence/template and opens the relevant target page for manual submission.
